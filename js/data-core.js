...
/* Pages through the whole table with .range() since Supabase caps any
   single request at 1000 rows by default. Returns a plain array of row
   objects, e.g. [{market:'ARIZONA', store_name:'...', qty:1, ...}, ...] */
async function fetchAllSupabaseRows(){
  ...
  while(true){
    ...
    rows.push(...data);
    if(data.length < SUPABASE_PAGE_SIZE) break; // last page
